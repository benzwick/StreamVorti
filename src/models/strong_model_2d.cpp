...
#include "StreamVorti/models/strong_model_2d.hpp"

namespace StreamVorti {

StrongModel2d::StrongModel2d() : is_mass_scaled_(false), min_mass_scale_factor_(0.), max_mass_scale_factor_(0.)
{}


StrongModel2d::~StrongModel2d()
{}


void StrongModel2d::LoadMeshRepresentation(const std::string &mesh_filename)
{
    // Load the mesh.
...
void StrongModel2d::CreateIntegrationPoints(const IntegOptions &options, const SupportDomain &support_dom)
{
...
void StrongModel2d::ComputeMass(const std::vector<double> &density, const std::vector<double> &time_steps, const double &max_time_step,
                              const std::vector< std::vector<int> > &support_nodes_ids, bool scaling)
{
    ...
    if (this->grid_.Nodes().size() == 0) {
        std::string error = "[ExplicitSim ERROR] Cannot compute 3d weak model's mass. The model's grid nodes are not available.";
        throw std::runtime_error(error.c_str());
    }
    ...
    if (scaling) {
        ...
        std::cout << "[ExplicitSim] Mass scaling: ON\n";
        ...
        for (auto &weight : this->integ_points_.Weights()) {
            ...
            for (auto &support_id : support_nodes_ids[i]) {
                ...
                this->mass_[support_id] += density[i] * scale_factor * weight;
            }
        ...
        for (auto &weight : this->integ_points_.Weights()) {
            ...
            for (auto &support_id : support_nodes_ids[i]) {
                ...
                this->mass_[support_id] += density[i] * weight;
            }
        } // End iteration over model's integration points.
